...
public class Server
{
     ...
     */
    private WebAppDeployer createWebAppDeployer(
    ...
    private int getPort()
    {
        return Integer.parseInt(properties.getProperty(KEY_SERVER_PORT,
                DEFAULT_SERVER_PORT));
    }

    private int getSecurePort()
    {
        return Integer.parseInt(properties.getProperty(KEY_SECURE_SERVER_PORT,
    ...
    {
        return properties.getProperty(KEY_KEYSTORE_LOC, sslSocketConnector
                .getKeystore());
    }

    private int getRequestLogRetainDays()
    {
        return Integer.parseInt(properties.getProperty(KEY_REQUEST_LOG_DAYS,
    ...
    private boolean isJoiningThreadPool()
    {
        return Boolean.parseBoolean(properties.getProperty(
                KEY_JOIN_THREAD_POOL, DEFAULT_JOIN_THREAD_POOL));
    }

    private boolean unpackWebArchive()
    {
        return Boolean.parseBoolean(properties.getProperty(KEY_UNPACK_WAR,
                DEFAULT_UNPACK_WAR));
    }

    private int getMaxThreads()
    {
        return Integer.parseInt(properties.getProperty(KEY_MAX_THREADS,
                DEFAULT_MAX_THREADS));
    }

    private int getMinThreads()
    {
        return Integer.parseInt(properties.getProperty(KEY_MIN_THREADS,
                DEFAULT_MIN_THREADS));
    }
    ...
    protected void startServer() throws Exception
    {
        if (!initialized)
            throw new RuntimeException(
                    "Server not initialized, please initialize!!");

        embeddedServer.start();
        // Only join the thread pool if the user explicitly states
        if (isJoiningThreadPool()) embeddedServer.join();
            ...
            throws IllegalArgumentException
    {
        ...
        OptionBuilder
                .withDescription("the name and location of the properties file to use");
